...
var boundoutSchema=new mongoose.Schema({
    unitname:String,
    snumber:String,
    sperson:String,
    yperson:String,
...
//建立查询用的模型
var boundout_model=mongoose.model('boundout',boundoutSchema,'boundout');
...
router.get('/getUsers', function(req, res, next) {
    boundoutSchema.find({},function(err,datas){
        if(!err){
            res.send(datas);
        }
    });
...
router.post('/add', function(req, res, next) {
    ...
    //2.处理请求
    var newBoundout =  new boundoutSchema();
    newBoundout.unitname = unitname;
    newBoundout.snumber = snumber;
    newBoundout.sperson = sperson;
    newBoundout.yperson = yperson;
    newBoundout.bperson = bperson;
    newBoundout.sremark = sremark;
...
router.get('/del', function(req, res, next) {
    ...
    idarr.forEach(function(obj,i){
        ...
        boundout_model.find({'_id':obj},function(err,datas){
            //console.log(datas);
...
router.get('/getone', function(req, res, next) {
    ...
    var id = req.query.id;
    console.log(id);
    boundout_model.findById(id,function(err,data){
        if(!err){
            res.send(data);
...
router.post('/edit', function(req, res, next) {
    ...
    //2.处理请求
    boundout_model.findById(id,function(err,datas){
        if(datas){
            datas._id=id;
...
router.get("/getdepart",function(req,res){
    ...
    // 2、处理
    boundout_model.find({},function(err, datas){
        //totalCount  数据总条数
